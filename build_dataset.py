...
for (i, imagePath) in enumerate(imagePaths):
    ...
    # load the contents of the annotation file, then initialize the
    ...
    #run selective search on the image to generate bounding box
    # proposals
    ...
    for (x, y, w, h) in rects:
        # convert the (x, y, w, h) bounding box coordinates to
    ...
    for proposedRect in proposedRects[:config.MAX_PROPOSALS]:
        #unpack the proposed rectangle bounding box coordinates
        (propStartX, propStartY, propEndX, propEndY) = proposedRect

        #loop over the ground-truth bounding boxes
        for gtBox in gtBoxes:
            #compute the intersection over union between the two
            # bounding boxes and initialize the list of ground-truth
            ...
            if not fullOverlap and iou < 0.05 and negativeROIs <= config.MAX_NEGATIVE:
                # extract the ROI and then derive the output path
                # to the negative ROI
                roi = image[propStartY:propEndY, propStartX:propEndX]
